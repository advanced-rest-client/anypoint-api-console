...
class AnypointConsoleBuild {
  ...
  install() {
    ...
    log.on('log', (info) => {
      ...
      console.log(`bower ${name}       ${info.id} ${info.message}`);
    });
    log.on('info', (info) => {
      let name;
      if (info.data && info.data.endpoint) {
      ...
      console.info(`bower ${name}       ${info.id} ${info.message}`);
    });
    log.on('warn', (info) => {
      console.warn(`bower       ${info.id} ${info.message}`);
    });
    log.on('error', (info) => {
      console.error(`bower       ${info.id} ${info.message}`);
    });
  ...
  build() {
    ...
    const promises = polymerProject.config.builds.map((buildOptions) => {
      return buildLib(buildOptions, polymerProject, this.logger);
    });
   ...
   */
  clearLocal() {
    this.logger.info('Clearing working directory output...');
  ...
  copyMissing() {
    ...
    const buildPath = path.join(this._workingDir, 'build');
    return this.getFolders(buildPath)
    .then((buildFolders) => {
      const libs = [];
      buildFolders.forEach((item) => {
        libs.push([
          path.join(this._workingDir, 'bower_components', 'prism', 'prism.js'),
